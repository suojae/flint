...
/// side effect를 직접 실행하지 마세요.
///
/// ## 원리
/// Flutter의 `build()`는 여러 번 호출되는 것이 정상입니다.
/// 여기서 `Navigator.push()`, `showDialog()`, `bloc.add()` 같은
/// side effect를 직접 실행하면 rebuild마다 같은 동작이 반복되어:
/// - 같은 화면이 여러 번 push되거나
/// - dialog/snackbar가 중복 표시되거나
/// - Bloc 이벤트가 반복 발행되어 무한 루프나 중복 요청으로 이어질 수 있습니다.
///
/// 이런 side effect는 `onPressed` 같은 사용자 액션 콜백,
/// `BlocListener`, `initState()`, `ref.listen`, post-frame callback 등
...
class AvoidSideEffectInBuild extends FlintLintRule {
  ...
  static const _code = LintCode(
    ...
    correctionMessage:
        'Move navigation, dialogs, snackbars, and bloc event dispatch '
        'to callbacks, listeners, initState(), or another effect boundary.',
  );
